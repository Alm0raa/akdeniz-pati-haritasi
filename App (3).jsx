...
function ProfilePage({onBack,onGoPoints,onOpenQr,onScores}){
 ...
 useEffect(()=>{
  ...
  supabase.auth.getSession().then(({data})=>{const current=data.session||null;setSession(current);if(current)loadProfile(current.user.id);else setLoading(false)});
  const{data:listener}=supabase.auth.onAuthStateChange((_event,current)=>{setSession(current);if(current)loadProfile(current.user.id);else{setProfile(null);setLoading(false)}});
  return()=>listener.subscription.unsubscribe();
 },[]);
 async function loadProfile(id){setLoading(true);const{data,error}=await supabase.from('profiles').select('*').eq('id',id).single();if(error)setError('Profil yüklenemedi: '+error.message);else setProfile(data);setLoading(false)}
 async function login(e){e.preventDefault();clear();setBusy(true);const{data,error}=await supabase.auth.signInWithPassword({email:form.email.trim(),password:form.password});if(error)setError(error.message==='Invalid login credentials'?'E-posta veya şifre hatalı.':error.message);else if(data.session){setSession(data.session);await loadProfile(data.session.user.id)}setBusy(false)}
 async function signup(e){e.preventDefault();clear();if(form.password.length<8){setError('Şifre en az 8 karakter olmalı.');return}if(form.password!==form.confirmPassword){setError('Şifreler eşleşmiyor.');return}if(!form.accepted){setError('Kullanım koşullarını kabul etmelisin.');return}setBusy(true);const{data,error}=await supabase.auth.signUp({email:form.email.trim(),password:form.password,options:{emailRedirectTo:window.location.origin,data:{first_name:form.firstName.trim(),last_name:form.lastName.trim(),faculty:form.faculty.trim(),department:form.department.trim()}}});if(error)setError(error.message);else if(data.session){setSession(data.session);await loadProfile(data.session.user.id)}else{setMessage('Hesabın oluşturuldu. Doğrulama bağlantısı için e-postanı kontrol et.');setMode('login')}setBusy(false)}
 ...
 async function saveProfile(e){e.preventDefault();clear();setBusy(true);const payload={first_name:profile.first_name.trim(),last_name:profile.last_name.trim(),faculty:profile.faculty?.trim()||null,department:profile.department?.trim()||null};if('paw_color'in profile)payload.paw_color=profile.paw_color||'#087A50';const{data,error}=await supabase.from('profiles').update(payload).eq('id',session.user.id).select().single();if(error)setError(error.message);else{setProfile(data);setMessage('Profil bilgilerin kaydedildi.')}setBusy(false)}
 async function chooseColor(color){const old=profile;setProfile({...profile,paw_color:color});const{error}=await supabase.from('profiles').update({paw_color:color}).eq('id',session.user.id);if(error){setProfile(old);setError('Pati rengi kaydedilemedi. Önce paw_color alanını Supabase’e eklemelisin.')}}
 async function logout(){await supabase.auth.signOut();setSession(null);setProfile(null);setMode('welcome')}
 const navBar=<nav>{nav.map(([Icon,label])=><button key={label} className={label==='Profil'?'activeNav':''} onClick={()=>label==='Ana'?onBack():label==='Mama Noktaları'?onGoPoints():label==='QR'?onOpenQr():label==='Puanlar'?onScores():null}><Icon/><span>{label}</span></button>)}</nav>;
 if(!supabaseConfigured)return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark"><PawPrint size={30}/></div><h1>Bağlantı kurulamadı</h1><p>Supabase bağlantı değişkenleri bulunamadı.</p></section>{navBar}</div></main>;
