...
function ProfilePage({onBack}){
 ...
 const[summary,setSummary]=useState({points:0,total:0,food:0,water:0,photo:0});
 ...
 async function loadProfile(id){setLoading(true);const[{data,error},{data:stats}]=await Promise.all([supabase.from('profiles').select('*').eq('id',id).single(),supabase.rpc('get_my_point_summary')]);if(error)setError('Profil bilgileri alınamadı: '+error.message);else{setProfile(data);setSummary(stats?.[0]||{points:0,total:0,food:0,water:0,photo:0});setAvatarPhoto(localStorage.getItem('pati-profile-photo-'+id)||'')}setLoading(false)}
 ...
 if(session&&profile){
  const initials=((profile.first_name?.[0]||'P')+(profile.last_name?.[0]||'')).toUpperCase();
  const level=levelFor(Number(summary.points||0));
  const location=[profile.faculty,profile.department].filter(Boolean).join(' • ');
  const choosePhoto=e=>{const file=e.target.files?.[0];if(!file)return;if(file.size>2*1024*1024){setError('Profil fotoğrafı 2 MB’dan küçük olmalı.');return}const reader=new FileReader();reader.onload=()=>{const value=String(reader.result);setAvatarPhoto(value);localStorage.setItem('pati-profile-photo-'+session.user.id,value)};reader.readAsDataURL(file)};
  if(profileView==='edit')return <main><div className="phone authPage"><ProfileHeader onBack={()=>setProfileView('home')}/>{message&&<div className="authNotice success">{message}</div>}{error&&<div className="authNotice error">{error}</div>}<form className="authForm profileForm" onSubmit={saveProfile}><AuthField label="Ad" icon={<UserRound/>}><input value={profile.first_name||''} onChange={e=>setProfile({...profile,first_name:e.target.value})} required/></AuthField><AuthField label="Soyad" icon={<UserRound/>}><input value={profile.last_name||''} onChange={e=>setProfile({...profile,last_name:e.target.value})} required/></AuthField><AuthField label="Fakülte (isteğe bağlı)" icon={<Building2/>}><input value={profile.faculty||''} onChange={e=>setProfile({...profile,faculty:e.target.value})}/></AuthField><AuthField label="Bölüm (isteğe bağlı)" icon={<BookOpen/>}><input value={profile.department||''} onChange={e=>setProfile({...profile,department:e.target.value})}/></AuthField><button className="authPrimary" disabled={busy}><Save/>{busy?'Kaydediliyor...':'Profili kaydet'}</button></form><ProfileBottomNav active="Profil"/></div></main>;
  return <main><div className="phone authPage profileHomePage"><ProfileHeader onBack={onBack}/><section className="profileHomeHero"><button className="profilePhoto" onClick={()=>photoInput.current?.click()}>{avatarPhoto?<img src={avatarPhoto} alt="Profil"/>:<span>{initials}</span>}<i><Camera/></i></button><input ref={photoInput} hidden type="file" accept="image/*" onChange={choosePhoto}/><h2>{profile.first_name} {profile.last_name}</h2><p>{session.user.email}</p><div className="profileLevel"><Trophy/><b>{level.name}</b><span>{summary.points||0} Pati Puanı</span></div>{location&&<small>{location}</small>}</section><section className="profileQuickStats"><div><b>{summary.total||0}</b><span>Katkı</span></div><div><b>{summary.food||0}</b><span>Mama</span></div><div><b>{summary.water||0}</b><span>Su</span></div></section><section className="profileMenu"><button onClick={()=>setProfileView('edit')}><UserRound/><span><b>Profil Bilgilerim</b><small>Ad, soyad, fakülte ve bölüm</small></span><ChevronRight/></button><button><Award/><span><b>Rozetlerim</b><small>Kazandığın rozetleri görüntüle</small></span><ChevronRight/></button><a href="mailto:akduhaysevpatiharitasi@gmail.com"><Mail/><span><b>İletişim ve Destek</b><small>İstek, öneri ve şikâyet</small></span><ChevronRight/></a><button className="profileLogout" onClick={logout}><LogOut/><span><b>Çıkış Yap</b><small>Hesabından güvenli şekilde çık</small></span></button></section><ProfileBottomNav active="Profil"/></div></main>;
 }
