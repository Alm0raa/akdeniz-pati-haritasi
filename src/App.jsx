...
import{Bell,Camera,Check,ChevronLeft,ChevronRight,CircleAlert,Download,Edit3,Expand,Home,ImagePlus,LayoutDashboard,Map,MapPin,Minus,PawPrint,Plus,QrCode,Save,ScanLine,Settings,Shield,Trash2,UserRound,Users,Utensils,X}from"lucide-react";
const seed=[
...
function FeedingPoints({points,onBack,onShowMap,onUpdate,onProfile}){
 ...
 return <main><div className="phone pointsPage"><header className="pointsHead"><button onClick={onBack}><ChevronLeft/></button><div><small>MAMA NOKTALARI</small><h1>Kampüs ihtiyaçları</h1><p>Öncelikli noktaları görüntüle ve hızlıca destek ol.</p></div></header><div className="filterRow"><button className={filter==='all'?'active':''} onClick={()=>setFilter('all')}>Tümü</button><button className={filter==='empty'?'active':''} onClick={()=>setFilter('empty')}>Acil</button><button className={filter==='urgent'?'active':''} onClick={()=>setFilter('urgent')}>Kontrol</button><button className={filter==='warning'?'active':''} onClick={()=>setFilter('warning')}>Azalıyor</button><button className={filter==='full'?'active':''} onClick={()=>setFilter('full')}>Dolu</button></div><div className="sortRow"><span>{filtered.length} nokta</span><select value={sort} onChange={e=>setSort(e.target.value)}><option value="priority">En acil</option><option value="newest">En son güncellenen</option><option value="name">A’dan Z’ye</option></select></div><section className="pointsList">{filtered.map(point=><article key={point.id} className="pointCard"><i className="statusLine" style={{background:S[point.status][1]}}/><div className="pointBody"><span className="pointBadge" style={{color:S[point.status][1],background:S[point.status][1]+'18'}}>{S[point.status][0]}</span><h2>{point.name}</h2><p><MapPin size={14}/>{point.area}</p><div className="pointMeta"><span><PawPrint size={14}/>{point.animals}</span><span>{ago(point.updated)}</span></div><div className="pointActions"><button onClick={()=>setDetail(point)}>Detaylar</button><button className="mapButton" onClick={()=>onShowMap(point)}><Map size={16}/> Haritada göster</button></div></div></article>)}</section><nav>{[[Home,'Ana'],[MapPin,'Mama Noktaları'],[QrCode,'QR'],[CircleAlert,'Acil'],[PawPrint,'Profil']].map(([Icon,label])=><button key={label} className={label==='Mama Noktaları'?'activeNav':''} onClick={()=>label==='Ana'?onBack():label==='Profil'?onProfile():null}><Icon/><span>{label}</span></button>)}</nav>{detail&&<Sheet point={detail} onClose={()=>setDetail(null)} onUpdate={update}/>}</div></main>
}

function ProfilePage({onBack,onGoPoints,onOpenQr}){
 const[mode,setMode]=useState('welcome');
 const[showPassword,setShowPassword]=useState(false);
 const[faculty,setFaculty]=useState('');
 const[pawColor,setPawColor]=useState(()=>localStorage.getItem('pati-avatar-color')||'#087A50');
 const[session,setSession]=useState(null);
 const[profile,setProfile]=useState(null);
 const[busy,setBusy]=useState(false);
 const[message,setMessage]=useState(null);
 const nav=[[Home,'Ana'],[MapPin,'Mama Noktaları'],[QrCode,'QR'],[CircleAlert,'Kupa'],[PawPrint,'Profil']];
 useEffect(()=>{
  if(!supabaseConfigured)return;
  supabase.auth.getSession().then(({data})=>{setSession(data.session);if(data.session)loadProfile(data.session.user.id)});
  const{data:listener}=supabase.auth.onAuthStateChange((_event,next)=>{setSession(next);if(next)loadProfile(next.user.id);else setProfile(null)});
  return()=>listener.subscription.unsubscribe();
 },[]);
 async function loadProfile(id){const{data}=await supabase.from('profiles').select('id,first_name,last_name,faculty,department,role,is_suspended').eq('id',id).maybeSingle();setProfile(data||null)}
 const notice=(type,text)=>setMessage({type,text});
 async function login(e){e.preventDefault();if(!supabaseConfigured)return notice('error','Supabase bağlantısı henüz ayarlanmadı.');setBusy(true);setMessage(null);const form=new FormData(e.currentTarget);const{error}=await supabase.auth.signInWithPassword({email:String(form.get('email')).trim(),password:String(form.get('password'))});setBusy(false);if(error)return notice('error',error.message==='Invalid login credentials'?'E-posta veya şifre hatalı.':error.message);notice('success','Giriş başarılı.');}
 async function register(e){e.preventDefault();if(!supabaseConfigured)return notice('error','Supabase bağlantısı henüz ayarlanmadı.');const form=new FormData(e.currentTarget);const password=String(form.get('password'));if(password!==String(form.get('confirmPassword')))return notice('error','Şifreler birbiriyle aynı değil.');setBusy(true);setMessage(null);const firstName=String(form.get('firstName')).trim(),lastName=String(form.get('lastName')).trim(),department=String(form.get('department')).trim();const{data,error}=await supabase.auth.signUp({email:String(form.get('email')).trim(),password,options:{emailRedirectTo:window.location.origin,data:{first_name:firstName,last_name:lastName,faculty:faculty||null,department:department||null}}});if(!error&&data.user&&data.session){await supabase.from('profiles').upsert({id:data.user.id,first_name:firstName,last_name:lastName,faculty:faculty||null,department:department||null,role:'member',is_suspended:false},{onConflict:'id'});await loadProfile(data.user.id)}setBusy(false);if(error)return notice('error',error.message);notice('success',data.session?'Hesabın oluşturuldu ve giriş yapıldı.':'Hesabın oluşturuldu. E-postana gelen doğrulama bağlantısına tıkla.');if(!data.session)setMode('login')}
 async function resetPassword(e){e.preventDefault();if(!supabaseConfigured)return notice('error','Supabase bağlantısı henüz ayarlanmadı.');setBusy(true);setMessage(null);const form=new FormData(e.currentTarget);const{error}=await supabase.auth.resetPasswordForEmail(String(form.get('email')).trim(),{redirectTo:window.location.origin});setBusy(false);notice(error?'error':'success',error?error.message:'Şifre yenileme bağlantısı e-postana gönderildi.')}
 async function logout(){setBusy(true);await supabase.auth.signOut();setBusy(false);setMode('welcome');notice('success','Çıkış yapıldı.')}
 const chooseColor=color=>{setPawColor(color);localStorage.setItem('pati-avatar-color',color)};
 if(session)return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark" style={{background:pawColor}}><PawPrint size={30}/></div><small>HAYSEV PATİ HARİTASI</small><h1>Merhaba, {profile?.first_name||session.user.user_metadata?.first_name||'Pati Dostu'}!</h1><p>{profile?.role==='admin'?'Ana Yönetici':profile?.role==='moderator'?'Moderatör':'Üye'} hesabınla giriş yaptın.</p></section><section className="pawPicker"><b>Pati rengini seç</b><div className="pawColors">{['#087A50','#7146D9','#1677E8','#F08A24','#E64B8C','#F04444'].map(color=><button key={color} type="button" aria-label="Pati rengi seç" className={pawColor===color?'selected':''} style={{background:color}} onClick={()=>chooseColor(color)}><PawPrint size={18}/></button>)}</div></section><section className="signedProfile"><div className="profileInfo"><b>{profile?.first_name||session.user.user_metadata?.first_name} {profile?.last_name||session.user.user_metadata?.last_name}</b><span>{session.user.email}</span>{(profile?.faculty||profile?.department)&&<small>{[profile?.faculty,profile?.department].filter(Boolean).join(' · ')}</small>}</div>{profile?.is_suspended&&<div className="authMessage error">Hesabın askıya alınmış. Destek ekibiyle iletişime geç.</div>}<button className="profileSecondary" onClick={logout} disabled={busy}>Çıkış Yap</button><div className="supportBox"><b>İletişim ve Destek</b><span>İstek, öneri ve şikâyetlerin için</span><a href="mailto:akduhaysevpatiharitasi@gmail.com">akduhaysevpatiharitasi@gmail.com</a></div></section><nav>{nav.map(([Icon,label])=><button key={label} className={label==='Profil'?'activeNav':''} onClick={()=>label==='Ana'?onBack():label==='Mama Noktaları'?onGoPoints():label==='QR'?onOpenQr():null}><Icon/><span>{label}</span></button>)}</nav></div></main>;
 return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark" style={{background:pawColor}}><PawPrint size={30}/></div><small>HAYSEV PATİ HARİTASI</small><h1>{mode==='login'?'Tekrar hoş geldin!':mode==='register'?'Aramıza katıl':mode==='forgot'?'Şifreni yenile':'Pati Haritası’na Katıl'}</h1><p>{mode==='welcome'?'Mama noktalarını güncellemek ve katkılarını takip etmek için hesabına giriş yap.':mode==='login'?'Kaldığın yerden devam etmek için bilgilerini gir.':mode==='forgot'?'E-postana güvenli bir yenileme bağlantısı gönderelim.':'Kampüsteki dostlarımız için birlikte hareket edelim.'}</p></section><section className="pawPicker"><b>Pati rengini seç</b><div className="pawColors">{['#087A50','#7146D9','#1677E8','#F08A24','#E64B8C','#F04444'].map(color=><button key={color} type="button" aria-label="Pati rengi seç" className={pawColor===color?'selected':''} style={{background:color}} onClick={()=>chooseColor(color)}><PawPrint size={18}/></button>)}</div></section>{message&&<div className={`authMessage ${message.type}`}>{message.text}</div>}{mode==='welcome'&&<section className="profileChoices"><button className="profilePrimary" onClick={()=>{setMessage(null);setMode('login')}}>Giriş Yap</button><button className="profileSecondary" onClick={()=>{setMessage(null);setMode('register')}}>Hesap Oluştur</button><button className="forgotLink" onClick={()=>{setMessage(null);setMode('forgot')}}>Şifremi unuttum</button><div className="supportBox"><b>İletişim ve Destek</b><span>İstek, öneri ve şikâyetlerin için</span><a href="mailto:akduhaysevpatiharitasi@gmail.com">akduhaysevpatiharitasi@gmail.com</a></div></section>}{mode==='login'&&<form className="authForm" onSubmit={login}><label>E-posta<input name="email" type="email" required placeholder="E-posta adresin"/></label><label>Şifre<div className="passwordField"><input name="password" type={showPassword?'text':'password'} required placeholder="Şifren"/><button type="button" onClick={()=>setShowPassword(v=>!v)}>{showPassword?'Gizle':'Göster'}</button></div></label><button className="profilePrimary" disabled={busy}>{busy?'Giriş yapılıyor...':'Giriş Yap'}</button><button type="button" className="forgotLink" onClick={()=>{setMessage(null);setMode('forgot')}}>Şifremi unuttum</button><p className="switchAuth">Hesabın yok mu? <button type="button" onClick={()=>{setMessage(null);setMode('register')}}>Hesap oluştur</button></p></form>}{mode==='register'&&<form className="authForm registerForm" onSubmit={register}><div className="nameGrid"><label>Ad<input name="firstName" required placeholder="Adın"/></label><label>Soyad<input name="lastName" required placeholder="Soyadın"/></label></div><label>E-posta<input name="email" type="email" required placeholder="E-posta adresin"/></label><label>Şifre<input name="password" type="password" minLength="8" required placeholder="En az 8 karakter"/></label><label>Şifre tekrarı<input name="confirmPassword" type="password" minLength="8" required placeholder="Şifreni yeniden yaz"/></label><div className="optionalTitle"><b>İsteğe bağlı bilgiler</b><span>Sonradan da ekleyebilirsin</span></div><label>Fakülte <em>İsteğe bağlı</em><select value={faculty} onChange={e=>setFaculty(e.target.value)}><option value="">Seçmek istemiyorum</option><option>İletişim Fakültesi</option><option>Fen Fakültesi</option><option>Mühendislik Fakültesi</option><option>Eğitim Fakültesi</option><option>Hukuk Fakültesi</option><option>Diğer</option></select></label><label>Bölüm <em>İsteğe bağlı</em><input name="department" placeholder="Bölümünü yazabilirsin"/></label><label className="consent"><input type="checkbox" required/><span>Kullanım koşullarını ve gizlilik bilgilendirmesini kabul ediyorum.</span></label><button className="profilePrimary" disabled={busy}>{busy?'Hesap oluşturuluyor...':'Hesap Oluştur'}</button><p className="switchAuth">Zaten hesabın var mı? <button type="button" onClick={()=>{setMessage(null);setMode('login')}}>Giriş yap</button></p></form>}{mode==='forgot'&&<form className="authForm forgotForm" onSubmit={resetPassword}><div className="forgotIcon">✉</div><label>E-posta<input name="email" type="email" required placeholder="E-posta adresin"/></label><button className="profilePrimary" disabled={busy}>{busy?'Gönderiliyor...':'Bağlantı Gönder'}</button><button type="button" className="forgotLink" onClick={()=>{setMessage(null);setMode('login')}}>Giriş ekranına dön</button></form>}<nav>{nav.map(([Icon,label])=><button key={label} className={label==='Profil'?'activeNav':''} onClick={()=>label==='Ana'?onBack():label==='Mama Noktaları'?onGoPoints():label==='QR'?onOpenQr():null}><Icon/><span>{label}</span></button>)}</nav></div></main>
}
