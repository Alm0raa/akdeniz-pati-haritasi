...
function ProfilePage({onBack,onGoPoints,onOpenQr}){
 ...
 const nav=[[Home,'Ana'],[MapPin,'Mama Noktaları'],[QrCode,'QR'],[CircleAlert,'Acil'],[PawPrint,'Profil']];
 const colors=['#087A50','#7146D9','#1677E8','#F08A24','#E64B8C','#F04444'];
 ...
 async function login(e){e.preventDefault();clear();setBusy(true);const{error}=await supabase.auth.signInWithPassword({email:form.email.trim(),password:form.password});if(error)setError(error.message==='Invalid login credentials'?'E-posta veya şifre hatalı.':error.message);setBusy(false)}
 async function signup(e){e.preventDefault();clear();if(form.password.length<8){setError('Şifre en az 8 karakter olmalı.');return}if(form.password!==form.confirmPassword){setError('Şifreler eşleşmiyor.');return}if(!form.accepted){setError('Kullanım koşullarını kabul etmelisin.');return}setBusy(true);const{data,error}=await supabase.auth.signUp({email:form.email.trim(),password:form.password,options:{emailRedirectTo:window.location.origin,data:{first_name:form.firstName.trim(),last_name:form.lastName.trim(),faculty:form.faculty.trim(),department:form.department.trim()}}});if(error)setError(error.message);else if(!data.session){setMessage('Hesabın oluşturuldu. Doğrulama bağlantısı için e-postanı kontrol et.');setMode('login')}setBusy(false)}
 async function forgot(e){e.preventDefault();clear();setBusy(true);const{error}=await supabase.auth.resetPasswordForEmail(form.email.trim(),{redirectTo:window.location.origin});if(error)setError(error.message);else setMessage('Şifre yenileme bağlantısı e-posta adresine gönderildi.');setBusy(false)}
 async function saveProfile(e){e?.preventDefault();clear();setBusy(true);const payload={first_name:profile.first_name.trim(),last_name:profile.last_name.trim(),faculty:profile.faculty?.trim()||null,department:profile.department?.trim()||null,paw_color:profile.paw_color||'#087A50'};const{data,error}=await supabase.from('profiles').update(payload).eq('id',session.user.id).select().single();if(error)setError(error.message);else{setProfile(data);setMessage('Profil bilgileri kaydedildi.')}setBusy(false)}
 async function chooseColor(color){const previous=profile;setProfile({...profile,paw_color:color});const{error}=await supabase.from('profiles').update({paw_color:color}).eq('id',session.user.id);if(error){setProfile(previous);setError('Pati rengi kaydedilemedi: '+error.message)}}
 async function logout(){await supabase.auth.signOut();setMode('welcome');setMessage('Oturum kapatıldı.')}
 const navBar=<nav>{nav.map(([Icon,label])=><button key={label} className={label==='Profil'?'activeNav':''} onClick={()=>label==='Ana'?onBack():label==='Mama Noktaları'?onGoPoints():label==='QR'?onOpenQr():null}><Icon/><span>{label}</span></button>)}</nav>;
 if(!supabaseConfigured)return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark"><PawPrint size={30}/></div><h1>Bağlantı kurulamadı</h1><p>Supabase bağlantı değişkenlerini kontrol et.</p></section>{navBar}</div></main>;
 if(loading)return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark"><PawPrint size={30}/></div><h1>Profil yükleniyor</h1></section>{navBar}</div></main>;
 if(session&&profile){const color=profile.paw_color||'#087A50';return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark" style={{background:color}}><PawPrint size={30}/></div><small>HAYSEV PATİ HARİTASI</small><h1>{profile.first_name} {profile.last_name}</h1><p>{session.user.email}</p></section>{message&&<div className="profileNotice success">{message}</div>}{error&&<div className="profileNotice error">{error}</div>}<section className="pawPicker"><b>Pati rengini seç</b><div className="pawColors">{colors.map(item=><button key={item} type="button" aria-label="Pati rengini seç" className={color===item?'selected':''} style={{background:item}} onClick={()=>chooseColor(item)}><PawPrint size={18}/></button>)}</div></section><form className="authForm" onSubmit={saveProfile}><div className="nameGrid"><label>Ad<input value={profile.first_name||''} onChange={e=>setProfile({...profile,first_name:e.target.value})} required/></label><label>Soyad<input value={profile.last_name||''} onChange={e=>setProfile({...profile,last_name:e.target.value})} required/></label></div><div className="optionalTitle"><b>İsteğe bağlı bilgiler</b></div><label>Fakülte <em>İsteğe bağlı</em><input value={profile.faculty||''} onChange={e=>setProfile({...profile,faculty:e.target.value})} placeholder="Fakülteni yazabilirsin"/></label><label>Bölüm <em>İsteğe bağlı</em><input value={profile.department||''} onChange={e=>setProfile({...profile,department:e.target.value})} placeholder="Bölümünü yazabilirsin"/></label><button className="profilePrimary" disabled={busy}>{busy?'Kaydediliyor...':'Profili Kaydet'}</button><button type="button" className="profileSecondary" onClick={logout}>Çıkış Yap</button></form>{navBar}</div></main>}
 return <main><div className="phone profilePage"><section className="profileTop"><div className="profileMark"><PawPrint size={30}/></div><small>HAYSEV PATİ HARİTASI</small><h1>{mode==='login'?'Tekrar hoş geldin!':mode==='register'?'Aramıza katıl':mode==='forgot'?'Şifreni yenile':'Pati Haritası’na Katıl'}</h1><p>{mode==='welcome'?'Mama noktalarını güncellemek ve katkılarını takip etmek için hesabına giriş yap.':mode==='login'?'Kaldığın yerden devam etmek için bilgilerini gir.':mode==='forgot'?'Şifre yenileme bağlantısı için e-posta adresini yaz.':'Kampüsteki dostlarımız için birlikte hareket edelim.'}</p></section>{message&&<div className="profileNotice success">{message}</div>}{error&&<div className="profileNotice error">{error}</div>}{mode==='welcome'&&<section className="profileChoices"><button className="profilePrimary" onClick={()=>setMode('login')}>Giriş Yap</button><button className="profileSecondary" onClick={()=>setMode('register')}>Hesap Oluştur</button><button className="forgotLink" onClick={()=>setMode('forgot')}>Şifremi unuttum</button><div className="supportBox"><b>İletişim ve Destek</b><span>İstek, öneri ve şikâyetlerin için</span><a href="mailto:akduhaysevpatiharitasi@gmail.com">akduhaysevpatiharitasi@gmail.com</a></div></section>}{mode==='login'&&<form className="authForm" onSubmit={login}><label>E-posta<input type="email" value={form.email} onChange={e=>change('email',e.target.value)} required placeholder="E-posta adresin" autoComplete="email"/></label><label>Şifre<div className="passwordField"><input type={showPassword?'text':'password'} value={form.password} onChange={e=>change('password',e.target.value)} required placeholder="Şifren" autoComplete="current-password"/><button type="button" onClick={()=>setShowPassword(v=>!v)}>{showPassword?'Gizle':'Göster'}</button></div></label><button className="profilePrimary" disabled={busy}>{busy?'Giriş yapılıyor...':'Giriş Yap'}</button><button type="button" className="forgotLink" onClick={()=>setMode('forgot')}>Şifremi unuttum</button><p className="switchAuth">Hesabın yok mu? <button type="button" onClick={()=>setMode('register')}>Hesap oluştur</button></p></form>}{mode==='register'&&<form className="authForm registerForm" onSubmit={signup}><div className="nameGrid"><label>Ad<input value={form.firstName} onChange={e=>change('firstName',e.target.value)} required placeholder="Adın"/></label><label>Soyad<input value={form.lastName} onChange={e=>change('lastName',e.target.value)} required placeholder="Soyadın"/></label></div><label>E-posta<input type="email" value={form.email} onChange={e=>change('email',e.target.value)} required placeholder="E-posta adresin"/></label><label>Şifre<input type="password" value={form.password} onChange={e=>change('password',e.target.value)} minLength="8" required placeholder="En az 8 karakter"/></label><label>Şifre tekrarı<input type="password" value={form.confirmPassword} onChange={e=>change('confirmPassword',e.target.value)} minLength="8" required placeholder="Şifreni yeniden yaz"/></label><div className="optionalTitle"><b>İsteğe bağlı bilgiler</b><span>Sonradan da ekleyebilirsin</span></div><label>Fakülte <em>İsteğe bağlı</em><input value={form.faculty} onChange={e=>change('faculty',e.target.value)} placeholder="Fakülteni yazabilirsin"/></label><label>Bölüm <em>İsteğe bağlı</em><input value={form.department} onChange={e=>change('department',e.target.value)} placeholder="Bölümünü yazabilirsin"/></label><label className="consent"><input type="checkbox" checked={form.accepted} onChange={e=>change('accepted',e.target.checked)} required/><span>Kullanım koşullarını ve gizlilik bilgilendirmesini kabul ediyorum.</span></label><button className="profilePrimary" disabled={busy}>{busy?'Hesap oluşturuluyor...':'Hesap Oluştur'}</button><p className="switchAuth">Zaten hesabın var mı? <button type="button" onClick={()=>setMode('login')}>Giriş yap</button></p></form>}{mode==='forgot'&&<form className="authForm forgotForm" onSubmit={forgot}><div className="forgotIcon">✉</div><h2>Şifreni yenile</h2><p>E-posta adresini yaz, şifre yenileme bağlantısını gönderelim.</p><label>E-posta<input type="email" value={form.email} onChange={e=>change('email',e.target.value)} required placeholder="E-posta adresin"/></label><button className="profilePrimary" disabled={busy}>{busy?'Gönderiliyor...':'Bağlantı Gönder'}</button><button type="button" className="forgotLink" onClick={()=>setMode('login')}>Giriş ekranına dön</button></form>}{navBar}</div></main>
}
